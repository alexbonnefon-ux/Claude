...
def rule_based_score(job: dict) -> float:
    ...
    comp_score = 2 if any(t in combined for t in TECH_TERMS) else 1

    total = role_score + loc_score + sal_score + comp_score + 0.5
...
def relative_date(date_str) -> str:
    if not date_str:
...
def remote_badge(policy: str) -> str:
    m = {
        "full":   '<span class="badge badge-remote">\U0001f30d Full Remote</span>',
        "hybrid": '<span class="badge badge-hybrid">\U0001f3e2 Hybride</span>',
        "onsite": '<span class="badge badge-onsite">\U0001f4cd Présentiel</span>',
    }
    return m.get(policy, '<span class="badge badge-unknown">❓ Non précisé</span>')
...
def job_card(job: dict) -> str:
    ...
    score_title = f"Score estimé (sans IA)" if estimated else f"Score IA {score}/10"
    ...
    label_map = {"role_match": "Rôle", "location_match": "Lieu", "salary_match": "Salaire", "company_match": "Entreprise", "freshness": "Fraîcheur"}
    bd = "".join(f"<div class='bd-row'><span>{v}</span><span>{details[k]}</span></div>" for k, v in label_map.items() if k in details)
    bd_html = f"<div class='score-breakdown'>{bd}</div>" if bd else ""
    title = job.get('title', '').replace('<', '&lt;').replace('>', '&gt;')
    company = job.get('company', '').replace('<', '&lt;').replace('>', '&gt;')
    location = job.get('location', '').replace('<', '&lt;').replace('>', '&gt;')
    url = job.get('url', '#')
    source = job.get('source', '')
    return f"""
    <div class="job-card">
      <div class="card-left"><div class="score-badge" style="background:{color}" title="{score_title}">{score_label}</div></div>
      <div class="card-body">
        <div class="job-title">{title}</div>
        <div class="job-company">{company}</div>
        <div class="job-meta">
          <span class="badge badge-location">\U0001f4cd {location}</span>
          {remote}
          <span class="badge badge-salary">\U0001f4b6 {sal}</span>
          <span class="badge badge-source">{source}</span>
        </div>{bd_html}
      </div>
      <div class="card-right">
        <div class="posted-date">{rel_date}</div>
        <a href="{url}" target="_blank" rel="noopener" class="apply-btn">Postuler →</a>
      </div>
    </div>"""


def section_html(title: str, emoji: str, color: str, jobs: list) -> str:
    header = f'<h2 class="section-title" style="color:{color}">{emoji} {title} <span class="count">{len(jobs)}</span></h2>'
    if not jobs:
        return f'<section class="section">{header}<div class="empty-section">Aucune offre dans cette catégorie aujourd\'hui.</div></section>'
    return f'<section class="section">{header}{chr(10).join(job_card(j) for j in jobs)}</section>'


def build_html(jobs: list, stats: dict, generated_at: datetime) -> str:
    ...
    total_db = stats.get("total") or len(jobs)
    gen_str = generated_at.strftime("%d/%m/%Y à %H:%M UTC")
    sections = (section_html("Top Picks", "\U0001f3c6", "#22c55e", top) +
                section_html("Bons Matchs", "✅", "#eab308", good) +
                section_html("À Surveiller", "\U0001f440", "#f97316", worth))
    return f"""<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>\U0001f3af HR Job Radar – Alexis Bonnefon</title>
<style>
:root{{--bg:#0f172a;--surface:#1e293b;--border:#334155;--text:#f1f5f9;--muted:#94a3b8;--green:#22c55e;--yellow:#eab308;--orange:#f97316;--blue:#3b82f6}}
...
.header-left p{{color:var(--muted);margin-top:4px;font-size:.9rem}}
.header-right{{text-align:right;font-size:.85rem;color:var(--muted)}}
...
.stat-label{{font-size:.75rem;color:var(--muted);margin-top:2px;text-transform:uppercase;letter-spacing:.05em}}
.trigger-bar{{padding:0 32px 20px;display:flex;align-items:center;gap:12px;flex-wrap:wrap}}
.trigger-note{{font-size:.85rem;color:var(--muted)}}
.trigger-link{{display:inline-flex;align-items:center;gap:6px;background:var(--blue);color:#fff;text-decoration:none;padding:8px 18px;border-radius:8px;font-weight:600;font-size:.9rem;transition:opacity .15s}}
.trigger-link:hover{{opacity:.85}}
main{{padding:0 32px 48px;max-width:1000px;margin:0 auto}}
.section{{margin-bottom:40px;margin-top:24px}}
.section-title{{font-size:1.15rem;font-weight:700;margin-bottom:16px;display:flex;align-items:center;gap:8px}}
.count{{background:var(--border);color:var(--muted);border-radius:999px;padding:2px 10px;font-size:.8rem;font-weight:600}}
...
.score-badge{{width:48px;height:48px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;font-weight:800;color:#0f172a;cursor:default}}
...
.job-company{{color:var(--muted);font-size:.88rem;margin-bottom:8px}}
.job-meta{{display:flex;flex-wrap:wrap;gap:6px}}
.badge{{font-size:.75rem;padding:3px 8px;border-radius:6px;font-weight:500;white-space:nowrap}}
...
.score-breakdown{{margin-top:8px;display:none;background:#0f1a2b;border-radius:8px;padding:8px 12px;font-size:.78rem;color:var(--muted)}}
...
.posted-date{{font-size:.78rem;color:var(--muted);white-space:nowrap}}
.apply-btn{{display:inline-block;background:var(--blue);color:#fff;text-decoration:none;padding:7px 14px;border-radius:8px;font-size:.82rem;font-weight:600;white-space:nowrap;transition:opacity .15s}}
.apply-btn:hover{{opacity:.85}}
footer{{text-align:center;padding:24px;color:var(--muted);font-size:.8rem;border-top:1px solid var(--border)}}
@media(max-width:600px){{header,.stats-bar,.trigger-bar,main{{padding-left:16px;padding-right:16px}}.card-right{{display:none}}.job-title{{white-space:normal}}}}
</style>
</head>
<body>
<header>
  <div class="header-left"><h1>\U0001f3af HR Job Radar</h1><p>Alexis Bonnefon · Tours, France · HRBP, DRH, People Manager</p></div>
  <div class="header-right"><strong>{len(jobs)} offres filtrées</strong>Mis à jour le {gen_str}</div>
</header>
<div class="stats-bar">
...
<div class="trigger-bar">
  <span class="trigger-note">⏰ Mis à jour chaque matin à 8h (Paris)</span>
  <a class="trigger-link" href="https://github.com/alexbonnefon-ux/Claude/actions/workflows/daily_scraper.yml" target="_blank" rel="noopener">▶ Lancer maintenant</a>
</div>
<main>{sections}</main>
<footer>Généré le {gen_str} · <a href="https://github.com/alexbonnefon-ux/Claude" style="color:var(--blue)">Code source</a><br><small>Scores avec ~ = estimés localement. Scores sans ~ = calculés par IA.</small></footer>
</body>
</html>"""
