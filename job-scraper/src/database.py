...
@dataclass
class Job:
    """Standardized job listing data class."""
    source: str
    job_id: str
    title: str
    company: str
    location: str
    url: str
    remote_policy: str = ""          # "full", "hybrid", "onsite", "unknown"
...
async def save_job(job: Job, db_path: str = DB_PATH) -> Optional[int]:
    ...
    Returns the row ID on success, None if the job already exists and was not updated.
    """
    async with aiosqlite.connect(db_path) as db:
        try:
            cursor = await db.execute("""
                ...
                ON CONFLICT(source, job_id) DO UPDATE SET
                    ...
                    salary_estimated = excluded.salary_estimated,
                    url = excluded.url,
                    posted_date = COALESCE(excluded.posted_date, jobs.posted_date),
                    score = COALESCE(excluded.score, jobs.score),
                    score_details = CASE
                        WHEN excluded.score IS NOT NULL THEN excluded.score_details
                        ELSE jobs.score_details
                    END,
                    description = CASE
            ...
            """, (
                ...
                job.salary_min,
                job.salary_max,
                1 if job.salary_estimated else 0,
                job.url,
                job.posted_date.isoformat() if job.posted_date else None,
                job.scraped_at.isoformat(),
                job.score,
                json.dumps(job.score_details),
                1 if job.emailed else 0,
...
async def get_unsent_jobs(min_score: float = 4.0, db_path: str = DB_PATH) -> List[dict]:
    """Retrieve scored, unsent jobs above the minimum score threshold."""
    async with aiosqlite.connect(db_path) as db:
...
async def update_job_score(
    ...
    """Update the score and score details for a job by its DB row ID."""
    async with aiosqlite.connect(db_path) as db:
        await db.execute("""
            UPDATE jobs SET score = ?, score_details = ?
            WHERE id = ?
...
async def mark_as_sent(job_ids: List[int], db_path: str = DB_PATH) -> None:
    """Mark a list of jobs (by DB row ID) as emailed."""
    if not job_ids:
        return
    async with aiosqlite.connect(db_path) as db:
